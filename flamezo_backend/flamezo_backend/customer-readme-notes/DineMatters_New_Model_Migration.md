# DineMatters — Business Model Migration & Backend Gap Analysis
...
### Overview
DineMatters operated as a **B2B SaaS platform** — sell software subscriptions to restaurants, charge commission on orders, charge for feature usage.
...
- Customers pay directly to restaurant (no DineMatters in the middle)
- DineMatters bills commission monthly via mandate on restaurant wallet
...
2. **No consumer play** — purely B2B, no reason for customers to use DineMatters over Zomato
...
### Overview
DineMatters pivots from **B2B SaaS** to a **two-sided restaurant platform**. The new model:
...
- Passes 2% gateway fee (Razorpay/Cashfree) to consumer or restaurant — DineMatters bears ₹0
...
| Peak-hour convenience fee | ₹10–15 flat | Consumer pays, DineMatters keeps |
...
Consumer places order
       ↓
DineMatters payment gateway (Razorpay/Cashfree)
       ↓
Total collected = Order Amount + 2% Gateway Fee (paid by consumer/restaurant)
       ↓
DineMatters deducts 1.5% commission (tracked in Monthly Billing Ledger)
       ↓
98.5% released to restaurant bank account
       ↓
Gateway 2% goes to Razorpay/Cashfree — DineMatters bears ₹0
```
...
- Earn FLAMEZO Points (loyalty coins) at every DineMatters restaurant
...
- DineMatters actively brings new customers via FLAMEZO app
- Dashboard shows: "DineMatters sent you X new customers this month"
...
| Who pays gateway fee | DineMatters absorbed (problem) | Consumer/restaurant — DineMatters ₹0 |
...
- But also: during trial, restaurants should still be able to process payments (that's how DineMatters earns commission)
...
- This means DineMatters is silently absorbing the gateway fee — wrong in new model

**New behavior:**
- Gateway fee (2%) added on top of order total before creating Razorpay order
- DineMatters commission (1.5%) tracked separately in order notes
...
- DineMatters bills separately at month end
...
- DineMatters collects full payment → immediately splits: 98.5% restaurant, 1.5% DineMatters
...
| Merchant dashboard (React) | Add "DineMatters Commission This Month" widget with GMV / commission / net breakdown |
...
- This is the #1 retention argument — restaurants see DineMatters working
...
*This document is the single source of truth for the DineMatters model migration. Update this file as decisions are made and gaps are closed.*
