...
def handle_product_update(doc, method=None):
    ...
    try:
        ...
        if getattr(restaurant, "enable_google_sync", False):
            ...
            frappe.enqueue(
                "flamezo_backend.flamezo_backend.api.google_business.sync_menu_to_google",
...
def fetch_all_restaurant_insights():
    ...
    for r in restaurants:
        frappe.enqueue(
            "flamezo_backend.flamezo_backend.api.google_business.fetch_google_insights",
...
def generate_review_reply(review_text, rating, restaurant_id=None):
    ...
    """
    from flamezo_backend.flamezo_backend.services.ai.base import get_openai_client, handle_ai_error
    try:
