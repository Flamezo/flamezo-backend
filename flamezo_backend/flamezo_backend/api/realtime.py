...
def notify_order_update(doc, method=None):
	...
	try:
		...
		if (doc.status or '').lower() in statuses_that_need_push:
			frappe.enqueue(
				'flamezo_backend.flamezo_backend.api.push_notifications.send_order_status_push_to_customer',
				order_name=doc.name,
				queue='short',
				timeout=30,
				is_async=True
...
def notify_new_order_to_merchant(doc, method=None):
	...
	try:
		if doc.status in ('confirmed', 'pending_verification', 'Pending Verification'):
			frappe.enqueue(
				'flamezo_backend.flamezo_backend.api.push_notifications.send_new_order_push_to_merchant',
				order_name=doc.name,
				queue='short',
				timeout=30,
				is_async=True
