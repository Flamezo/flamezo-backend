...
from flamezo_backend.flamezo_backend.utils.razorpay_utils import get_razorpay_config, get_razorpay_client, get_or_create_razorpay_customer
...
def create_payment_order(restaurant_id, order_items, total_amount, subtotal=None, packaging_fee=0, delivery_fee=0, customer_name=None, customer_email=None, customer_phone=None, table_number=None, existing_order_id=None, idempotency_key=None, order_type=None, coupon_code=None, loyalty_coins_redeemed=0, delivery_info=None, pickup_time=None, tax=None, cgst=None, sgst=None, tax_percent=None):
	...
	try:
		...
		if not order_doc:
			...
			from flamezo_backend.flamezo_backend.api.orders import generate_order_number
		...
		if redeemed_coins > 0 and platform_customer:
			try:
				from flamezo_backend.flamezo_backend.utils.loyalty import get_loyalty_balance, is_loyalty_enabled
		...
		if coupon_code:
			try:
				from flamezo_backend.flamezo_backend.api.coupons import validate_coupon
...
def process_loyalty_and_coupons(order):
	...
	"""
	from flamezo_backend.flamezo_backend.utils.loyalty import redeem_loyalty_coins, earn_loyalty_coins
