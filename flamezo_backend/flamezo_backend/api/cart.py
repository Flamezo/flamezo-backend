...
def get_cart(restaurant_id, session_id=None, coupon_code=None, loyalty_coins=0, order_type=None, latitude=None, longitude=None):
	...
	try:
		...
		customer_id = None
		if user:
			from flamezo_backend.flamezo_backend.utils.customer_helpers import get_platform_customer_from_user
...
def parse_qr_code(qr_data):
	...
	POST /api/method/flamezo_backend.flamezo_backend.api.cart.parse_qr_code
	"""
	try:
		if not qr_data:
			return {
