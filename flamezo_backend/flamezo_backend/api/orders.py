...
from flamezo_backend.flamezo_backend.utils.api_helpers import (
	validate_restaurant_for_api, 
	validate_product_belongs_to_restaurant,
	get_product_from_id
)
from flamezo_backend.flamezo_backend.utils.customization_helpers import load_product_customizations, validate_customizations
from flamezo_backend.flamezo_backend.utils.currency_helpers import get_restaurant_currency_info
...
def create_order(restaurant_id, items, cooking_requests=None, customer_info=None, delivery_info=None, session_id=None, table_number=None, coupon_code=None, payment_method=None, order_type=None, packaging_fee=None, delivery_fee=None, pickup_time=None, loyalty_coins_redeemed=0, referral_id=None, tax=None, cgst=None, sgst=None, tax_percent=None):
	...
	try:
		...
		if order_type == "dine_in" and table_number:
			from flamezo_backend.flamezo_backend.api.cart import parse_table_number_from_qr
		...
		try:
			from flamezo_backend.flamezo_backend.utils.loyalty import earn_loyalty_coins, redeem_loyalty_coins, add_loyalty_coins
			...
			if loyalty_prog and platform_customer:
				...
				if order_doc.referral_link:
					...
					if order_count == 0:
						...
						from flamezo_backend.flamezo_backend.api.loyalty import process_referral_welcome_bonus
		...
		from flamezo_backend.flamezo_backend.api.cart import clear_cart
...
def get_orders(restaurant_id, status=None, page=1, limit=20, session_id=None, admin_mode=False, date_from=None, date_to=None, search_query=None, recent_only=False, phone=None):
	...
	try:
		...
		if phone:
			from flamezo_backend.flamezo_backend.utils.customer_helpers import normalize_phone
	...
	try:
		...
		if include_items and order_list:
			...
			if product_names:
				...
				from flamezo_backend.flamezo_backend.api.products import format_products_for_listing
...
def _format_order_items_light(order_doc):
	...
	from flamezo_backend.flamezo_backend.api.products import format_product
	items = []
...
def update_order_feedback(order_id, customer_rating=None, customer_feedback=None):
	...
	try:
		...
		from flamezo_backend.flamezo_backend.utils.permission_helpers import get_user_restaurant_ids
...
def get_admin_orders(restaurant_id, status=None, page=1, limit=20, date_from=None, date_to=None, search_query=None):
	"""
	GET /api/method/flamezo_backend.flamezo_backend.api.orders.get_admin_orders
