...
from flamezo_backend.flamezo_backend.utils.api_helpers import validate_restaurant_for_api, get_restaurant_from_id
from flamezo_backend.flamezo_backend.utils.feature_gate import require_plan
from flamezo_backend.flamezo_backend.utils.customer_helpers import get_customer_token, get_customer_from_token
import json
import csv
import io
...
def get_coupons(restaurant_id, active_only=True):
	"""
	GET /api/method/flamezo_backend.flamezo_backend.api.coupons.get_coupons
...
def export_coupons(restaurant_id):
	"""
	GET /api/method/flamezo_backend.flamezo_backend.api.coupons.export_coupons
...
def import_coupons(restaurant_id, csv_content, overwrite_existing=False):
	"""
	POST /api/method/flamezo_backend.flamezo_backend.api.coupons.import_coupons
...
def generate_coupon_suggestions(restaurant_id, tone="attractive", offer_type_filter=None, count=6):
	"""
	POST /api/method/flamezo_backend.flamezo_backend.api.coupons.generate_coupon_suggestions
	...
	try:
		...
		from flamezo_backend.flamezo_backend.services.ai.coupon_generator import (
			generate_suggestions, FREE_MONTHLY_QUOTA, _check_quota_status,
		)
		from flamezo_backend.flamezo_backend.api.coin_billing import deduct_coins
...
def get_applicable_offers(restaurant_id, cart_items, cart_total, customer_id=None, order_type=None):
	"""
	POST /api/method/flamezo_backend.flamezo_backend.api.coupons.get_applicable_offers
