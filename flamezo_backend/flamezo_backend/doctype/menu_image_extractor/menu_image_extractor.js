	...
	frappe.confirm(
		...
		function() {
			...
			const proceed_with_extraction = () => {
				...
				console.log('📞 Calling server method: flamezo_backend.flamezo_backend.doctype.menu_image_extractor.menu_image_extractor.extract_menu_data');
				
				frappe.call({
					method: 'flamezo_backend.flamezo_backend.doctype.menu_image_extractor.menu_image_extractor.extract_menu_data',
					args: {
...
function approve_extracted_data(frm) {
	...
	frappe.confirm(
		...
		function() {
			...
			frappe.call({
				method: 'flamezo_backend.flamezo_backend.doctype.menu_image_extractor.menu_image_extractor.approve_extracted_data',
				args: {
					docname: frm.doc.name
				},
				freeze: true,
...
function generate_recommendations(frm) {
	...
	frappe.confirm(
		...
		function() {
			...
			frappe.call({
				method: 'flamezo_backend.flamezo_backend.doctype.menu_image_extractor.menu_image_extractor.generate_recommendations',
				args: {
					docname: frm.doc.name
				},
				freeze: true,
