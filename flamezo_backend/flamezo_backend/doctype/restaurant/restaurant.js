...
frappe.ui.form.on('Restaurant', {
	refresh: function(frm) {
		...
		if (frm.doc.tables && frm.doc.tables > 0) {
			...
			frappe.call({
				method: 'flamezo_backend.flamezo_backend.doctype.restaurant.restaurant.get_qr_codes_pdf_url',
				args: {
						...
						frm.add_custom_button(__('Delete QR Codes PDF'), function() {
							frappe.confirm(
								...
								function() {
									// Yes - delete the PDF
									frappe.call({
										method: 'flamezo_backend.flamezo_backend.doctype.restaurant.restaurant.delete_qr_codes_pdf',
										args: {
											restaurant: frm.doc.name
										},
										freeze: true,
						...
						frm.add_custom_button(__('Generate QR Codes'), function() {
							frappe.confirm(
								...
								function() {
									// Yes
									frappe.call({
										method: 'flamezo_backend.flamezo_backend.doctype.restaurant.restaurant.generate_qr_codes_pdf',
										args: {
											restaurant: frm.doc.name
										},
										freeze: true,
