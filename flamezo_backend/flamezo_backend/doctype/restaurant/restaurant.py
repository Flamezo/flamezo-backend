...
from flamezo_backend.flamezo_backend.doctype.restaurant.qr_branding import build_table_qr_assets, generate_pdf_from_assets
...
class Restaurant(Document):
	...
	def auto_assign_owner(self):
		...
		try:
			...
			from flamezo_backend.flamezo_backend.utils.permissions import assign_user_to_restaurant
	...
	def generate_table_qr_codes_pdf(self, layout="2x2", background_image=None, qr_type="dine_in"):
		...
		try:
			import time
			from flamezo_backend.flamezo_backend.doctype.restaurant.qr_branding import (
	...
	def get_qr_codes_pdf_url(self):
		...
		if existing_file and existing_file.file_url:
			...
			try:
				from flamezo_backend.flamezo_backend.media.storage import verify_object_exists
				import os
	...
	def get_special_qr_assets(self, force=False):
		from flamezo_backend.flamezo_backend.doctype.restaurant.qr_branding import build_special_qr_assets
...
def get_qr_scan_analytics(restaurant, days=30):
	...
	try:
		...
		from flamezo_backend.flamezo_backend.doctype.restaurant.qr_branding import SPECIAL_QR_CONFIGS
