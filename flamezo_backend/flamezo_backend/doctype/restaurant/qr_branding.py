...
from flamezo_backend.flamezo_backend.utils.common import safe_log_error, resolve_and_fetch_media
...
def upload_content_bytes(content, suffix, object_key, content_type):
	from flamezo_backend.flamezo_backend.media.storage import upload_object
...
def ensure_table_qr_assets(restaurant_doc, table_number, force=False, branding=None, logo_bytes=None, background_image_bytes=None, override_background_url=None):
	from flamezo_backend.flamezo_backend.media.storage import get_cdn_url, verify_object_exists
...
def generate_pdf_from_assets(assets, layout="2x2"):
	...
	import tempfile
	from io import BytesIO
	from flamezo_backend.flamezo_backend.media.storage import download_object
