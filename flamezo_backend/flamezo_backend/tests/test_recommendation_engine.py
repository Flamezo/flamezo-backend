...
def _get_engine(mock_frappe=None):
    ...
    with patch.dict("sys.modules", {"frappe": mock_frappe}):
        with patch("flamezo_backend.flamezo_backend.services.ai.base.get_openai_client"):
            from flamezo_backend.flamezo_backend.services.ai.recommendations import RecommendationEngine
...
class TestCoOrderMatrix(unittest.TestCase):
    ...
    def _import(self):
        from flamezo_backend.flamezo_backend.tasks.recommendation_tasks import _compute_co_order_matrix
...
class TestLogCoOrderEvents(unittest.TestCase):
    ...
    def _run(self, order, mock_frappe=None):
        ...
        with patch.dict("sys.modules", {"frappe": mock_frappe, "frappe.utils": mock_frappe_utils}):
            from flamezo_backend.flamezo_backend.tasks.recommendation_tasks import log_co_order_events
