...
Run with:
    bench run-tests --app flamezo_backend --module flamezo_backend.flamezo_backend.tests.test_loyalty
"""
...
from flamezo_backend.flamezo_backend.tests.utils import (
    make_restaurant,
    make_loyalty_config,
    make_customer,
    make_loyalty_entry,
...
class TestGetLoyaltyBalance(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        from flamezo_backend.flamezo_backend.utils.loyalty import get_loyalty_balance
...
class TestEarnLoyaltyCoins(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        from flamezo_backend.flamezo_backend.utils.loyalty import earn_loyalty_coins
    ...
    def test_expiry_date_set_correctly(self):
        ...
        from flamezo_backend.flamezo_backend.utils.platform_config import get_expiry_months
...
class TestRedeemLoyaltyCoins(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        from flamezo_backend.flamezo_backend.utils.loyalty import redeem_loyalty_coins
...
class TestSettleLoyaltyPoints(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        from flamezo_backend.flamezo_backend.utils.loyalty import settle_loyalty_points
...
class TestHandleOrderCancellation(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        from flamezo_backend.flamezo_backend.utils.loyalty import handle_order_cancellation
...
class TestHandleLoyaltySettlement(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        from flamezo_backend.flamezo_backend.utils.loyalty import handle_loyalty_settlement
...
class TestGetLoyaltyConfigFieldAllowlist(unittest.TestCase):
    ...
    def test_config_does_not_expose_internal_fields(self):
        from flamezo_backend.flamezo_backend.api.loyalty import get_loyalty_config
    ...
    def test_config_returns_platform_fixed_earn_percentage(self):
        ...
        from flamezo_backend.flamezo_backend.api.loyalty import get_loyalty_config
        from flamezo_backend.flamezo_backend.utils.platform_config import get_earn_percentage
        response = get_loyalty_config(self._res)
        data = response.get("data", {})
...
class TestGetLoyaltyTier(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        from flamezo_backend.flamezo_backend.utils.loyalty import get_loyalty_tier
...
class TestBirthdayBonusScheduler(unittest.TestCase):
    ...
    def test_no_bonus_without_loyalty_history(self):
        ...
        from flamezo_backend.flamezo_backend.tasks.loyalty_tasks import grant_birthday_bonuses
...
class TestSendCoinCreditPush(unittest.TestCase):
    ...
    def test_no_push_when_no_tokens(self):
        ...
        with patch("flamezo_backend.flamezo_backend.api.push_notifications._send_fcm_message") as mock_send:
            from flamezo_backend.flamezo_backend.utils.loyalty import send_coin_credit_push
    ...
    def test_push_sent_for_each_token(self):
        ...
        with patch("flamezo_backend.flamezo_backend.api.push_notifications._send_fcm_message", return_value=True) as mock_send:
            from flamezo_backend.flamezo_backend.utils.loyalty import send_coin_credit_push
    ...
    def test_stale_tokens_removed(self):
        ...
        with patch("flamezo_backend.flamezo_backend.api.push_notifications._send_fcm_message", side_effect=mock_send):
            from flamezo_backend.flamezo_backend.utils.loyalty import send_coin_credit_push
    ...
    def test_correct_title_format(self):
        ...
        with patch("flamezo_backend.flamezo_backend.api.push_notifications._send_fcm_message", side_effect=capture_send):
            from flamezo_backend.flamezo_backend.utils.loyalty import send_coin_credit_push
...
class TestTrackReferralVisitNoReward(unittest.TestCase):
    ...
    def test_visit_recorded_but_no_coins_awarded(self):
        ...
        from flamezo_backend.flamezo_backend.api.loyalty import track_referral_visit
        ip = "10.0.0.99"
...
class TestClaimReferralReward(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        # Referrer
        from flamezo_backend.flamezo_backend.utils.customer_helpers import get_or_create_customer
    ...
    def _call_claim(self, phone=None):
        ...
        from flamezo_backend.flamezo_backend.api.loyalty import claim_referral_reward
        phone = phone or self._referee_phone
        with patch("flamezo_backend.flamezo_backend.utils.customer_helpers.validate_customer_session", return_value=True), \
             patch("flamezo_backend.flamezo_backend.api.loyalty.validate_customer_session", return_value=True), \
             patch("flamezo_backend.flamezo_backend.api.loyalty.get_customer_token", return_value="mock-token"):
    ...
    def test_restaurant_mismatch_rejected(self):
        ...
        try:
            from flamezo_backend.flamezo_backend.api.loyalty import claim_referral_reward
            with patch("flamezo_backend.flamezo_backend.api.loyalty.validate_customer_session", return_value=True), \
                 patch("flamezo_backend.flamezo_backend.api.loyalty.get_customer_token", return_value="mock-token"):
...
class TestCentralizedLoyaltyModel(unittest.TestCase):
    ...
    def test_get_config_platform_tier_thresholds(self):
        ...
        from flamezo_backend.flamezo_backend.api.loyalty import get_loyalty_config
        from flamezo_backend.flamezo_backend.utils.platform_config import PLATFORM_LOYALTY
    ...
    def test_update_config_strips_earn_percentage_override(self):
        ...
        from flamezo_backend.flamezo_backend.api.loyalty import update_loyalty_config
        from flamezo_backend.flamezo_backend.utils.platform_config import get_earn_percentage
    ...
    def test_platform_config_max_coins_plan_tiered(self):
        from flamezo_backend.flamezo_backend.utils.platform_config import get_max_coins_per_order
    ...
    def test_platform_config_redemption_percent_plan_tiered(self):
        from flamezo_backend.flamezo_backend.utils.platform_config import get_max_redemption_percent
    ...
    def test_platform_config_birthday_bonus_plan_tiered(self):
        from flamezo_backend.flamezo_backend.utils.platform_config import get_birthday_bonus_coins
    ...
    def test_platform_config_referral_coins_is_40(self):
        from flamezo_backend.flamezo_backend.utils.platform_config import (
    ...
    def test_platform_config_max_opens_is_10(self):
        from flamezo_backend.flamezo_backend.utils.platform_config import get_max_opens_rewarded_per_share
...
class TestMonthlyReferralCycleReset(unittest.TestCase):
    ...
    def test_monthly_reset_zeroes_all_links(self):
        from flamezo_backend.flamezo_backend.tasks.loyalty_tasks import reset_referral_cycles_monthly
...
class TestManualAdjustmentCap(unittest.TestCase):
    ...
    def test_adjustment_within_cap_succeeds(self):
        from flamezo_backend.flamezo_backend.api.loyalty import adjust_customer_points
...
class TestPricingUsesGlobalBalance(unittest.TestCase):
    ...
    def test_coins_earned_at_restaurant_a_visible_in_cart_at_restaurant_b(self):
        ...
        from flamezo_backend.flamezo_backend.utils.pricing import calculate_cart_totals
...
class TestResetReferralCycleRemoved(unittest.TestCase):
    ...
    def test_reset_referral_cycle_not_importable(self):
        ...
        import flamezo_backend.flamezo_backend.api.loyalty as loyalty_api
        self.assertFalse(
    ...
    def test_orders_py_does_not_import_reset_referral_cycle(self):
        ...
        import inspect
        import flamezo_backend.flamezo_backend.api.orders as orders_mod
...
class TestCoinExpiryNotifications(unittest.TestCase):
    ...
    def test_no_expiring_coins_runs_cleanly(self):
        ...
        from flamezo_backend.flamezo_backend.tasks.loyalty_tasks import send_coin_expiry_notifications
        try:
...
class TestLoyaltyAnalytics(unittest.TestCase):
    ...
    def _call(self):
        from flamezo_backend.flamezo_backend.api.loyalty import get_loyalty_analytics
