...
Run all:
    bench run-tests --app flamezo_backend --module flamezo_backend.flamezo_backend.tests.test_ai_coupon_generator

Run only unit tests (no Gemini key needed):
    bench run-tests --app flamezo_backend --module flamezo_backend.flamezo_backend.tests.test_ai_coupon_generator.TestQuotaTracking
"""
...
class TestQuotaTracking(unittest.TestCase):
    ...
    def test_initial_quota_status_is_zero(self):
        from flamezo_backend.flamezo_backend.services.ai.coupon_generator import _check_quota_status, FREE_MONTHLY_QUOTA
    ...
    def test_increment_quota_increments_used(self):
        from flamezo_backend.flamezo_backend.services.ai.coupon_generator import _check_and_increment_quota, FREE_MONTHLY_QUOTA
    ...
    def test_check_status_does_not_increment(self):
        from flamezo_backend.flamezo_backend.services.ai.coupon_generator import _check_quota_status
...
class TestValidateAndClean(unittest.TestCase):
    ...
    def _validate(self, raw, tone="attractive"):
        from flamezo_backend.flamezo_backend.services.ai.coupon_generator import _validate_and_clean_suggestion
...
class TestBuildPrompt(unittest.TestCase):
    ...
    def _get_prompt(self, tone="attractive", offer_type_filter=None, count=6):
        from flamezo_backend.flamezo_backend.services.ai.coupon_generator import _get_restaurant_context, _build_prompt
    ...
    def test_context_has_correct_stats(self):
        from flamezo_backend.flamezo_backend.services.ai.coupon_generator import _get_restaurant_context
...
class TestGenerateSuggestionsWithMock(unittest.TestCase):
    ...
    def _run(self, tone="attractive", offer_type_filter=None):
        ...
        with patch(
            "flamezo_backend.flamezo_backend.services.ai.coupon_generator.get_gemini_client",
            return_value=mock_model,
        ):
            from flamezo_backend.flamezo_backend.services.ai import coupon_generator
    ...
    def test_invalid_json_response_returns_error(self):
        ...
        mock_model = MagicMock()
        mock_model.generate_content.return_value = mock_response
        with patch("flamezo_backend.flamezo_backend.services.ai.coupon_generator.get_gemini_client", return_value=mock_model):
            from flamezo_backend.flamezo_backend.services.ai import coupon_generator
    ...
    def test_quota_exceeded_returns_error(self):
        from flamezo_backend.flamezo_backend.services.ai.coupon_generator import FREE_MONTHLY_QUOTA
        current_month = now_datetime().strftime("%Y-%m")
...
class TestGenerateCouponSuggestionsAPI(unittest.TestCase):
    ...
    def _call(self, tone="attractive", offer_type_filter=None, count=3):
        ...
        with patch("flamezo_backend.flamezo_backend.services.ai.coupon_generator.get_gemini_client", return_value=mock_model):
            from flamezo_backend.flamezo_backend.api.coupons import generate_coupon_suggestions
    ...
    def test_get_ai_coupon_quota_endpoint(self):
        from flamezo_backend.flamezo_backend.api.coupons import get_ai_coupon_quota
...
class TestLiveGeneration(unittest.TestCase):
    ...
    def _generate(self, tone="attractive", offer_type_filter=None):
        from flamezo_backend.flamezo_backend.services.ai.coupon_generator import generate_suggestions
