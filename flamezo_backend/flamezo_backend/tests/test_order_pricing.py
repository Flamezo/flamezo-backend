...
Run with:
    bench run-tests --app flamezo_backend --module flamezo_backend.flamezo_backend.tests.test_order_pricing
"""
...
class TestProcessLoyaltyAndCouponsIdempotency(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        from flamezo_backend.flamezo_backend.api.payments import process_loyalty_and_coupons
...
class TestVerifyPaymentIdempotency(unittest.TestCase):
    ...
    def _create_test_order(self, rzp_order_id):
        ...
        with patch("flamezo_backend.flamezo_backend.api.realtime.notify_new_order_to_merchant"):
            order.insert(ignore_permissions=True)
        frappe.db.commit()
        return order

    @patch("flamezo_backend.flamezo_backend.api.payments.get_razorpay_client")
    @patch("flamezo_backend.flamezo_backend.api.payments.process_loyalty_and_coupons")
    def test_payment_status_set_to_completed(self, mock_process_loyalty, mock_get_client):
        ...
        from flamezo_backend.flamezo_backend.api.payments import verify_payment
...
class TestCoinPurchaseGST(unittest.TestCase):
    ...
    @patch("flamezo_backend.flamezo_backend.api.coin_billing.get_razorpay_client")
    @patch("flamezo_backend.flamezo_backend.api.coin_billing.get_razorpay_config")
    def test_18_percent_gst_on_1000(self, mock_cfg, mock_client):
        ...
        from flamezo_backend.flamezo_backend.api.coin_billing import create_coin_purchase_order
