...
def confirm_upload(upload_id, owner_doctype, owner_name, media_role, alt_text=None, caption=None, display_order=0):
	...
	if existing_asset:
		...
		if not url and asset_doc.raw_object_key:
			from flamezo_backend.flamezo_backend.media.storage import get_cdn_url
	...
	frappe.enqueue(
		"flamezo_backend.flamezo_backend.media.jobs.process_media_asset",
...
def delete_media_asset(media_id):
	...
	# Enqueue cleanup job
	frappe.enqueue(
		"flamezo_backend.flamezo_backend.media.jobs.cleanup_deleted_media",
