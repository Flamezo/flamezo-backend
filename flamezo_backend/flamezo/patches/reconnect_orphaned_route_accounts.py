...
def execute():
    for name_like, account_id in ORPHANS.items():
        rname = frappe.db.get_value("Restaurant", {"restaurant_name": ["like", name_like]}, "name")
        if not rname:
            continue  # restaurant not on this site
        if frappe.db.get_value("Restaurant", rname, "razorpay_account_id"):
            continue  # already linked — never overwrite a real value
        frappe.db.set_value("Restaurant", rname, {
