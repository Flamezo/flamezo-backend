...
def set_test_razorpay_customer(restaurant_name, customer_id, token_id=None):
    ...
    frappe.db.set_value("Outlet", restaurant_name, "razorpay_customer_id", customer_id)
    if token_id:
        frappe.db.set_value("Outlet", restaurant_name, "razorpay_token_id", token_id)
        frappe.db.set_value("Outlet", restaurant_name, "mandate_status", "active")
    frappe.db.commit()
