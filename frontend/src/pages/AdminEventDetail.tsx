...
export default function AdminEventDetail() {
  ...
  const [attendeesAvailable, setAttendeesAvailable] = useState(false)
  const [attendees, setAttendees] = useState<any[]>([])
  const [tab, setTab] = useState<'details' | 'attendees'>('details')

  const { call: fetchEvent } = useFrappePostCall('flamezo_backend.flamezo.api.admin.admin_get_event_detail')

  useEffect(() => {
    if (!id) return
    let alive = true
    setLoading(true)
    fetchEvent({ event_id: id })
      .then((res: any) => {
        ...
        if (data?.event) {
          ...
          setOutletId(data.outlet || '')
          setAttendeesAvailable(!!data.attendees_available)
  ...
  if (!event) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center gap-4">
        ...
        <Button variant="outline" onClick={() => navigate('/admin/events')}>Back to Events</Button>
      </div>
    )
  }
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6 max-w-5xl mx-auto">
      {/* Back + header */}
      <div>
        <Button variant="ghost" size="sm" className="gap-1.5 -ml-2 mb-3 text-muted-foreground" onClick={() => navigate('/admin/events')}>
          ...
          <div className="min-w-0">
            ...
            {outletId ? (
              <button
                ...
                onClick={() => navigate(`/admin/restaurants/${encodeURIComponent(outletId)}`)}
                title="Open merchant"
              >
                <Store className="h-4 w-4" /> {outletName || outletId}
              </button>
            ) : (
              <span className="mt-1 inline-flex items-center gap-1.5 text-sm text-muted-foreground italic">
                <Store className="h-4 w-4" /> Platform event (no merchant)
              </span>
            )}
          </div>
      ...
      {tab === 'details' ? (
          ...
          <div className="md:col-span-2 space-y-4">
            <Card>
              <CardContent className="p-5 grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <Detail icon={Store} label="Organised by" value={outletName || 'Platform event'} />
        ...
        <Card>
          <CardContent className="py-16 flex flex-col items-center text-center gap-3">
            ...
            <p className="text-sm text-muted-foreground max-w-md">Customers who tap “Join” on this event in the app will appear here — each clickable through to their full profile.</p>
          </CardContent>
        </Card>
      ) : (
        <Card>
          ...
          <CardContent className="pt-0 divide-y">
            {attendees.map((a, i) => (
              <button
                key={a.id || i}
                type="button"
                onClick={() => a.id && navigate(`/admin/customers/${encodeURIComponent(a.id)}`)}
                className={cn('w-full flex items-center justify-between gap-3 py-3 text-left', a.id && 'hover:bg-muted/40 -mx-2 px-2 rounded-md transition-colors')}
              >
                <div className="min-w-0">
